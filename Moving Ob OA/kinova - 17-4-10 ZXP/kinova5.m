...
%for generating obstacle points in t
for i = 1:11
    [xob_plot(i),yob_plot(i),zob_plot(i)] = Obstacle_t((T/10)*(i-1));
end

figure;
plot3(xob_plot,yob_plot,zob_plot,'or','Markersize',10,'linewidth',2);hold on;
...
for j=1:length(t),
%     xob_t = xob_tjj(j); yob_t = xob_tjj(j); zob_t = xob_tjj(j);
%     plot3(xob_t,yob_t,zob_t,'k*');hold on;
...
figure;
% plot3(xob_tjj,yob_tjj,zob_tjj,'k*');hold on;
