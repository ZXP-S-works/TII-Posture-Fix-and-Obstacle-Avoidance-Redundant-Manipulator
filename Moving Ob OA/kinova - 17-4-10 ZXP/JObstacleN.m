function [JN,distobf,rightB,effective_oan,distob]=JObstacleN(q,dq,t)
% ZXP JN=>Jo
...
[xob_t,yob_t,zob_t] = Obstacle_t(t);
...
% ZXP lenob critical point �������ϵ�λ��
% ZXP j_ob() critical point ��λ��
% ZXP distob critical point���� obstacle�ľ���
    for i=2:7, 
        OJs=(xob_t-x(i-1))^2+(yob_t-y(i-1))^2+(zob_t-z(i-1))^2;%s means square  %ZXP ON^2
        OCs=(xob_t-x(i))^2+(yob_t-y(i))^2+(zob_t-z(i))^2;%C: next joint  %ZXP OM^2
    ...
    if (distob(1)<=d2),%previously (distob(1)<=d2), wrong?
        ...
        dirob=sign([jxob(1)-xob_t;jyob(1)-yob_t;jzob(1)-zob_t]);
        JNew=-[dirob(1)*JO(1,:);dirob(2)*JO(2,:);dirob(3)*JO(3,:)];
        JN=[JN;JNew];distobf=[distobf;distob(1)];   %ZXP JN��Jo��distobf��d��������
        % if distob(1)==d2%(distob(1)==d2)    ZXP distobΪ���������������õ���d2��
        if (abs(distob(1)-d2)<=(10^-3*d2))
        %if abs(distob(1+(j-1)*4)-d2)<=(10^-4*d2)%(distob(1)==d2) 
            newB(1,:)=myabs(JNew*dq)';
            rightB=[rightB;newB(1,:)']; %ZXP rightB = [rox1,roy1,roz1,rox2,roy2,roz2...] critical point ��'�ٶ�'
        else
    ...
    if (distob(2)<=d2),
        ...
        dirob=sign([jxob(2)-xob_t;jyob(2)-yob_t;jzob(2)-zob_t]);
        JNew=-[dirob(1)*JO(1,:);dirob(2)*JO(2,:);dirob(3)*JO(3,:)];
    ...
    if (distob(3)<=d2),
        ...
        dirob=sign([jxob(3)-xob_t;jyob(3)-yob_t;jzob(3)-zob_t]);
        JNew=-[dirob(1)*JO(1,:);dirob(2)*JO(2,:);dirob(3)*JO(3,:)];
...
%        if (lenK(4)-lenob(4))<=d1, disp('too close to the end');end;
        dirob=sign([jxob(4)-xob_t;jyob(4)-yob_t;jzob(4)-zob_t]);
        JNew=-[dirob(1)*JO(1,:);dirob(2)*JO(2,:);dirob(3)*JO(3,:)];
...
%        if (lenK(5)-lenob(5))<=d1, disp('too close to the end');end;
        dirob=sign([jxob(5)-xob_t;jyob(5)-yob_t;jzob(5)-zob_t]);
        JNew=-[dirob(1)*JO(1,:);dirob(2)*JO(2,:);dirob(3)*JO(3,:)];
...
%       if abs(distob(4+(j-1)*4)-d2)<=(10^-4*d2)%(distob(1)==d2) 
    ...
    if (distob(6)<=d2),
        ...
        dirob=sign([jxob(6)-xob_t;jyob(6)-yob_t;jzob(6)-zob_t]);
        JNew=-[dirob(1)*JO(1,:);dirob(2)*JO(2,:);dirob(3)*JO(3,:)];
